...
public class Node<T> {
  ...
  public boolean inserirOrdenado(T v) {
    if (v.toString().compareTo(this.getValor().toString()) < 0) {
      ...
      if (this.getFilho_esq() != null)
        return this.getFilho_esq().inserirOrdenado(v);// senao for nulo continua navegando na arvore ate encontrar uma
                                                      // folha
      else {
  ...
  protected void imprimeEmLarguraRecursivo(Queue<Node<T>> f) {
    if (!f.isEmpty()) {
      Node<T> no = (Node<T>) f.poll();
